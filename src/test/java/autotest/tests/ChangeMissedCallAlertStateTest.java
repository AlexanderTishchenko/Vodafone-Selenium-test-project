...
import autotest.sites.enterprise.pages.basket.BasketThankYouPage;
import autotest.sites.enterprise.pages.HomeLoggedInPage;
import autotest.sites.enterprise.pages.MainLoggedInPage;
import autotest.sites.enterprise.pages.myservices.CallingAndSmsPage;
import autotest.logger.CustomLog;
import autotest.models.users.User;
import autotest.models.users.Users;
import autotest.sites.anonymous.pages.HomePage;
import autotest.sites.anonymous.pages.LogoutSuccessPage;
import autotest.utilities.PageOpening;
import autotest.utilities.UriBuilder;
...
public class ChangeMissedCallAlertStateTest extends SeleniumTestBase {
    ...
    public void test() {
        ...
        CustomLog.info("Credit for telephone number: " + credit);
        ...
        Assert.assertThat("Success message on 'basket' page is not equal with expected", basketPage.getSuccessMessageText(),
                is(equalTo("The service Missed calls alert was added to the list of changes")));

        BasketThankYouPage basketThankYouPage = basketPage.confirmChanges();
        Assert.assertThat("Success message on 'basket Thank You' page is not equal with expected", basketThankYouPage.getSuccessMessageText(),
                is(equalTo("Your request is being processed. We will send you an SMS once we finish your changes.")));
        ...
        Assert.assertThat("Service state of 'Missed Calls Alert' is not equal with expected", callingAndSmsPage.getMissedCallsAlertServiceStateText(), is((equalTo("PROCESSING"))));

        CustomLog.step(7, "Log out");
        LogoutSuccessPage logoutSuccessPage = callingAndSmsPage.logOut();
        Assert.assertThat("Header on 'Logout Success' page is not equal with expected", logoutSuccessPage.getHeaderText(),
                is(equalTo("You were successfully logged out from My Vodafone.")));
        Assert.assertThat("Message text on 'Logout Success' page is not equal with expected", logoutSuccessPage.getMessageText(),
                is(equalTo("If you haven't completed everything you wanted, you may log in again.")));
    }
}
